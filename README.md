...
- **多相机配置管理**：增删改相机配置，自动保存到 JSON（jar 所在目录的 `backup-config.json`）；添加相机时默认勾选"使用 FTPS"
...
- 按钮与说明文本统一配有 **ikonli 图标**，随深色/浅色主题自动变色

**② Jobx 导出工具**

- 一键流程：FTP/FTPS **递归枚举**相机上的全部 `.jobx` → 经 **CogSocket HMI** 连接（自动切换为离线并等待加载完成）→ 逐个加载作业 → 读取 `A0:Z599` 范围内全部单元格的**值与表达式** → 导出完成后**自动恢复原作业与在线状态**
- **相机列表批量导出**：维护相机列表（增删改，自动保存到 jar 目录的 `export-config.json`），多选后一键逐台导出，输出到 `输出目录/相机名称/yyyyMMddHHmmss/`，单台失败不影响其余相机；也可使用上方表单手动填写单个相机参数导出
- 每个作业生成一个独立 xlsx，含「单元格」与「位置布局」两个工作表，**A4 纵向、打印友好**
- 可勾选「跳过表达式（更快）」：只读单元格值，不逐格回读表达式
- 支持配置 HMI 端口（新固件 **80**、旧固件 **8087**、模拟器为随机端口）与 FTPS/TLS；输出默认在 jar 目录下 `exports/yyyyMMddHHmmss/`（批量导出时为 `exports/相机名称/yyyyMMddHHmmss/`）
...
```bash
./bundle-exe-with-timestamp.sh
```

生成根目录 `jobx文件备份助手_<时间戳>.exe`（约 135 MB，单文件内嵌资源启用压缩），这是一个自包含的原生启动器
（net8.0-windows 自包含单文件，工控机无需预装 Java / .NET），内嵌了 OpenJDK JRE 25 安装包
（`launcher/*.msi`）与应用 fat jar。首次运行时：

1. 检测本机是否存在 Java 25+（JAVA_HOME → PATH → 常见安装目录）；
2. 没有则以管理员权限静默安装内嵌的 JRE 25 MSI（未提权时自动弹出 UAC 提权），并设置 `JAVA_HOME`；
3. 把内嵌 fat jar 释放到 `%LOCALAPPDATA%\CognexJobxBackupTool\App\` 并启动。

注意：通过 exe 启动时，配置文件（`backup-config.json` / `export-config.json` / `config.json`）与默认备份 / 导出
目录都位于上述 `%LOCALAPPDATA%` 目录下，而不是 exe 所在目录。
...
| ikonli（core / swing / fontawesome5） | 12.3.1 | 按钮与说明文本图标（随主题变色） |
| QuickJS（cn.net.zhijian.quickjs） | 0.2.2 | JS 脚本引擎（本地 jar，运行需 JDK 19+） |
| Gson / slf4j-simple | 2.10.1 / 2.0.13 | 配置 JSON 序列化 / 日志 |
| .NET 8 WinForms 启动器 | net8.0-windows | 单文件 exe 启动器（内嵌 JRE 25 MSI + fat jar，自包含发布） |
...
│   ├── util/AppIcons.java     # 应用图标（窗口 / 任务栏多尺寸图标）；Icons.java：ikonli 图标助手
...
│   ├── config/ExportConfigManager.java  # export-config.json（jar 所在目录，UTF-8）
│   ├── model/ExportCamera.java# 导出相机配置（含 HMI 端口）
...
单文件 exe 启动器位于 `launcher/`（`Launcher/` 为 WinForms 项目，`bundle-exe-with-timestamp.sh`
为打包入口；`launcher/build-launcher.sh` 为兼容转发）。

### 关键行为与约定

- 配置文件一律写入 **jar 所在目录**（通过 `CodeSource` 定位），不是工作目录：备份用 `backup-config.json`，导出相机列表用 `export-config.json`，编辑器用 `config.json`；备份目录留空默认 `backups/`，导出默认 `exports/yyyyMMddHHmmss/`（批量导出为 `exports/相机名称/yyyyMMddHHmmss/`）。
- FTP 客户端固定 UTF-8 控制编码并开启 autodetectUTF8；连接超时 10s、数据超时 30s；被动模式 + 二进制传输；FTPS 为显式 TLS，登录后执行 `PBSZ 0` / `PROT P`；备份工具添加相机对话框默认勾选"使用 FTPS"。
...
- 相机密码以**明文**形式保存在 `backup-config.json` / `export-config.json` / `config.json` 中，请勿将这些文件提交到版本库或随意外发。
...
- **Multi-camera configuration management**: add / edit / remove camera configs, auto-saved to JSON (`backup-config.json` in the jar's directory); "Use FTPS" is checked by default when adding a camera
...
- Buttons and hint labels carry consistent **ikonli icons** that follow the dark/light theme
...
- **Camera-list batch export**: maintain a camera list (add / edit / remove, auto-saved to `export-config.json` in the jar's directory), multi-select and export them one by one with a single click — output goes to `output-dir/CameraName/yyyyMMddHHmmss/`, and one camera failing does not stop the rest; alternatively fill in the form above to export a single camera manually
...
- Configurable HMI port (new firmware **80**, legacy firmware **8087**, emulator uses a random port) and FTPS/TLS; output defaults to `exports/yyyyMMddHHmmss/` under the jar's directory (`exports/CameraName/yyyyMMddHHmmss/` for batch export)
...
This produces `jobx文件备份助手_<timestamp>.exe` (≈ 135 MB, with bundle compression enabled) in the project root — a self-contained
native launcher (net8.0-windows, self-contained single file; the target PC needs neither Java nor .NET
preinstalled) with the OpenJDK JRE 25 installer (`launcher/*.msi`) and the application fat jar embedded.
On first run it:

1. Detects whether Java 25+ exists on the machine (JAVA_HOME → PATH → common install locations);
2. If not, silently installs the embedded JRE 25 MSI with admin rights (auto UAC elevation), and sets `JAVA_HOME`;
3. Extracts the embedded fat jar to `%LOCALAPPDATA%\CognexJobxBackupTool\App\` and launches it.

Note: when launched via the exe, config files (`backup-config.json` / `export-config.json` / `config.json`) and the default
backup / export directories live in the `%LOCALAPPDATA%` folder above, not next to the exe.
...
| ikonli (core / swing / fontawesome5) | 12.3.1 | Button & hint icons (follow the theme colors) |
...
| .NET 8 WinForms launcher | net8.0-windows | Single-file exe launcher (embeds JRE 25 MSI + fat jar, self-contained) |
...
│   ├── model/                 # CameraConfig, AppSettings
│   ├── util/AppIcons.java     # App icons (multi-size window / taskbar icons); Icons.java: ikonli icon helpers
│   └── ui/                    # MainFrame (3-tab container), BackupTabPanel, CameraDialog, About/Usage
├── export/                    # Tab 2: Export tool
│   ├── ExportTabPanel.java    # Manual parameter form + camera-list batch export + log, background thread
│   ├── ExportTask.java        # Enumerate → HMI → offline → read values/expressions → xlsx → restore
│   ├── ExportCameraDialog.java# Add/edit export camera dialog
│   ├── config/ExportConfigManager.java  # export-config.json (jar's directory, UTF-8)
│   ├── model/ExportCamera.java# Export camera config (incl. HMI port)
...
The single-file exe launcher lives in `launcher/` (`Launcher/` is the WinForms project;
`bundle-exe-with-timestamp.sh` at the project root is the packaging entry point, and
`launcher/build-launcher.sh` is a compatibility shim).

### Key Behaviors & Conventions

- Config files are always written to the **jar's directory** (located via `CodeSource`), not the working directory: `backup-config.json` for backup, `export-config.json` for the export camera list, `config.json` for the editor; an empty backup directory defaults to `backups/`, export output defaults to `exports/yyyyMMddHHmmss/` (`exports/CameraName/yyyyMMddHHmmss/` for batch export).
- The FTP client always uses UTF-8 control encoding with autodetectUTF8 enabled; connect timeout 10s, data timeout 30s; passive mode + binary transfer. FTPS is explicit TLS, with `PBSZ 0` / `PROT P` executed after login; "Use FTPS" is checked by default when adding a camera in the backup tool.
...
- Camera passwords are stored in **plain text** in `backup-config.json` / `export-config.json` / `config.json`. Do not commit these files to version control or share them carelessly.
